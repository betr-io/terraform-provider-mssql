...
After that you can import the SQL Server database role using the server URL and `login name`, e.g.

```shell
terraform import mssql_role.example 'mssql://example-sql-server.database.windows.net/master/user@example.com'
```
